  ...
  def fightCard(self, attacker: int, cheat: bool = True, defender = None):
    ...
    if defender == None:
      if card.title == "bigtwig":
        defender = self.chooseCards("Creature", "Choose an enemy creature to attack:", "enemy", condition = lambda x: (x.taunt or not (True in [y.taunt for y in x.neighbors(self)])) and x.stun, con_message = "This minion is protected by taunt and/or is not stunned.")[0]
      elif card.title == "niffle_ape":
        defender = self.chooseCards("Creature", "Choose an enemy minion to attack:", "enemy")[0]
      else:
        defender = self.chooseCards("Creature", "Choose an enemy creature to attack:", "enemy", condition = lambda x: x.taunt or not (True in [y.taunt for y in x.neighbors(self)]), con_message = "This minion is protected by taunt.")[0]
    if defender == None:
      logging.error("No defender selected.")
      return
    try:
