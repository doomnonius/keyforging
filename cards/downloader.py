...
## Downloader taken from: https://github.com/Tyrox1/KeyForge-Downloader/blob/master/downloader.py
## Small modifications were made as a I saw fit

SAVE_DIRECTORY = "card-fronts"  # The folder where the images get saved
...
for exp_id in EXPANSIONS:
  ...
  if not os.path.exists(f"{SAVE_DIRECTORY}/{exp_id}"):
    os.makedirs(f"{SAVE_DIRECTORY}/{exp_id}")
...
for exp_id in EXPANSIONS:
  expansion = EXPANSIONS[exp_id]
  save_dir = f"{SAVE_DIRECTORY}/{exp_id}"
  ...
  if card_count < expansion["count"]:
    ...
    cards_missing = False
    print(f"==== Download of expansion {expansion['name']} already completed ====")
  ...
  while cards_missing:
    ...
    for card in decks["_linked"]["cards"]:  # Get the info for every card
      ...
      filename = f"{save_dir}/{card_title.lower().replace(' ', '-')}.png"  # local filename to save the image
