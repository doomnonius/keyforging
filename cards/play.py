  ...
  if len(inactiveBoard) > 1:
    ...
    if len(inactiveBoard) > 1: # at this point only creatures tied for highest power will be in play
      ...
      while len(inactiveBoard > 1):
        ...
        if choice == 0:
          card = inactiveBoard[1]
          destroy(card, game.inactivePlayer, game)
          if card.destroyed:
            game.pendingReloc.append(card)
        else:
          card = inactiveBoard[0]
          destroy(card, game.inactivePlayer, game)
          if card.destroyed:
            game.pendingReloc.append(card)
          choice -= 1
...
def hebe_the_huge (game, card):
  ...
  # deal damage
  [x.damageCalc(game, 2) for x in activeBoard if x.damage == 0 and activeBoard[x] != card]
  # deal damage
...
def hand_of_dis (game, card):
  ...
  choice = game.chooseCards("Creature", "Choose a non-flank creature to destroy:", condition = lambda x: x.isFlank(game), con_message = "You didn't choose a flank creature. Please try again.")[0]
  if choice[0] == "fr":
    card = active[choice[1]]
    destroy(card, game.activePlayer, game)
    if card.destroyed:
...
def honorable_claim (game, card):
  ...
  passFunc(game, card)
  active = game.activePlayer.board["Creature"]
  knights = sum("Knight" in x.text for x in active)
  if knights > game.inactivePlayer.amber and game.inactivePlayer.amber > 0:
    ...
    choices = [x[1] for x in game.chooseCards("Creature", "You have more knights than your opponent has amber. Choose which knights will capture an amber:", "friend", game.inactivePlayer.amber, condition = lambda x: "Knight" in x.text, con_message = "That's not a knight.")]
    for choice in choices:
...
def shield_of_ustice (game, card):
...
def horseman_of_pestilence (game, card):
  ...
  for x in active:
    if "Horseman" not in x.traits:
      x.damageCalc(game, 1)
      x.updateHealth(game.activePlayer)
      if x.destroyed:
        pending.append(x)
  for x in inactive:
    if "Horseman" not in x.traits:
      x.damageCalc(game, 1)
      x.updateHealth(game.inactivePlayer)
      if x.destroyed:
        pending.append(x)
